...
int main(int argc, char** argv)
{
    ...
    hsize_t chunk_dims[ndims] = {111, 111, 111, 3};
    char *data = new char[total_size];

    hid_t status = H5Dread(dset, H5T_NATIVE_CHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
    ...
    file =  H5Fcreate(new_file.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, fapl);
    ...
    dset = H5Dcreate(file, dset_name.c_str(), H5T_NATIVE_CHAR, space_id, H5P_DEFAULT, dcpl_id, 
                        H5P_DEFAULT);
    status = H5Dwrite(dset, H5T_NATIVE_CHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
