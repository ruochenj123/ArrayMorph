...
{
    hsize_t size = stoi(argv[1]);
    
    cout << "chunk size: " << size << endl;
    hsize_t X = 1, Y = size / 4;
    ...
    hid_t vol_id = H5VLregister_connector_by_name("s3_vol_hybrid", H5P_DEFAULT);
    ...
    Aws::InitAPI(options);
    string filename = "lambda_test/" + to_string(size) + ".h5";
