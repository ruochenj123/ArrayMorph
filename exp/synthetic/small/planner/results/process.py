...
re = [['Date', 'Platform', 'Query', 'Chunk Size (MB)', 'Transferred Size (Byte)','Total Time (s)','Cost ($)', 'Lambda Time (ms)']]
    ...
    for c in content:
        if c.startswith('../build'):
            ...
            if r[1] == "S3":
                ...
                lambda_time += gclogquery.query(start_stamp, end_stamp)
                time.sleep(5)
