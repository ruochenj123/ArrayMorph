...
p1<-ggplot(data, aes(Platform, Time)) + 
  ...
  scale_fill_manual(" ", 
                    labels = c("HSDS", "TileDB", "HSLAMBDA", "ArrayMorph"),
                    values = c("HSDS" = "black", "TileDB"="grey30", "HSLAMBDA" = "grey60", "ArrayMorph"="white")) +
...
xlab('Platform') +
ylab('Cost($)') +
scale_y_continuous(limits=c(0,5), breaks=c(0,1,2,3,4,5), expand=c(0,0))
...
ggsave(file="bert-1.eps", plot=p, width=45, height=12, device="eps")
